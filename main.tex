...
Como visto na Figura \ref{fig:confidence_threshold} o número de detecções se aproxima do {\it ground truth} no limiar de confiança $0.7$, abaixo disso a quantidade de detecções supera o número de tumores anotados, simultaneamente a partir de $0.8$ este mesmo número se torna risível. O que nos permite inferir que existe viabilidade de papel de apoio de um modelo classificador de objetos em situações médicas, pois as performances comparadas entre humano e máquina são similares apesar de não idênticas.
