...
Após a avaliação de resultados gerados conseguimos obter a partir da matriz de confusão a acurácia de $81\%$, o que é bastante satisfatório. Não obstante, é preciso levar em consideração algumas outras métricas para a compreensão completa da performance do modelo. Ao se observar o balanço entre os gráficos de {\it precision} e {\it recall} entende-se as detecções relevantes estão até o nível de confiança de $0.7$, a partir daí o {\it recall} decai de forma expressiva indicando que o modelo já não mais apresenta assertividade em suas detecções.
