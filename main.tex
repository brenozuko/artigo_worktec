...
Ademais, cada {\it bounding box} é composta por 5 valores: $x, y, w, h$ e confiança \cite{Redmon2015}. Onde $x, y$ são as coordenadas do centro da caixa delimitadora, $w, h$ são a largura e altura relativas a imagem completa.

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.7\textwidth]{images/yolo.png}
  \caption{Imagem retirada do artigo de \citeonline{Redmon2015}. Onde é possível ver com clareza o processo de separação da imagem em pequenas células em um grid $S x S$, a plotagem de $B$ {\it bounding boxes} e a detecção final .}
  \label{fig:yolo}
\end{figure}
